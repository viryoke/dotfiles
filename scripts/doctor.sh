...
check "nix flakes" "nix flake --help"
...
check "jupyter" "command -v jupyter"
...
if [ "$OS" = "Linux" ]; then
  ...
  warn "AUR helper (yay/paru)" "command -v yay || command -v paru"
  warn "flatpak" "command -v flatpak"
  warn "clash-verge" "command -v clash-verge"
  warn "ollama" "command -v ollama"
  warn "tailscale" "command -v tailscale"
elif [ "$OS" = "Darwin" ]; then
